...
void Roster::print_average_days_in_course(string student_id){
    for (int i = 0; i < roster_counter; i++) {
        if(class_roster_array[i]->get("student_id") == student_id){
            int avg_days = (stoi(class_roster_array[i]->get("first_day")) + stoi(class_roster_array[i]->get("second_day")) + stoi(class_roster_array[i]->get("third_day")))/3;
            cout << "Student ID: " << class_roster_array[i]->get("student_id") << ", average days in course is: " << avg_days << endl;
            break;
        }
    }
};

void Roster::parse(const string array[], int size){
...
void Roster::print_invalid_emails(){
    for(int i = 0; i < roster_counter; i++){
        if(!is_valid(class_roster_array[i]->get("email"))){
            cout << class_roster_array[i]->get("email");
            cout << " - is invalid."  << endl;
        }
    }
};

void Roster::print_by_degree_program(degree_program degree){
    degree_program checker = degree;
    string prnt;
    if(degree == SOFTWARE){
        prnt = "SOFTWARE";
    ...
    cout << " \n Showing students in degree program: " << prnt << endl;
    cout << "" << endl;
    
    for (int i = 0; i < roster_counter; i++) {
        if(class_roster_array[i]->get_degree_program() == checker){
            cout << class_roster_array[i]->get("student_id");
            cout << "\t";
            cout << class_roster_array[i]->get("first_name");
            cout << "\t";
            cout << class_roster_array[i]->get("last_name");
            cout << "\t";
            cout << class_roster_array[i]->get("age");
            cout << "\t";
            cout << class_roster_array[i]->get("days_to_complete");
            cout << "\t";
            cout << class_roster_array[i]->get("degree_program");
            cout << "" << endl;
        }
    }
};